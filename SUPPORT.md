# Support

This document describes how to get help with `stellar-flow`.

---

## Before Asking for Help

Please try the following first:

1. **Read the documentation** — start with the [README](README.md) and the
   [`docs/`](docs/) directory.
2. **Check the [FAQ](FAQ.md)** — your question may already be answered.
3. **Search existing issues** —
   [github.com/stellarbuild/stellar-flow/issues](https://github.com/stellarbuild/stellar-flow/issues)
4. **Check the [Troubleshooting guide](docs/troubleshooting.md)** for common
   errors and resolutions.

---

## Support Channels

### GitHub Discussions (Recommended)

For questions, ideas, and general discussion:

[github.com/stellarbuild/stellar-flow/discussions](https://github.com/stellarbuild/stellar-flow/discussions)

| Category | Purpose |
|---|---|
| **Q&A** | Ask questions about using the library |
| **Ideas** | Suggest features or improvements |
| **Show and Tell** | Share projects built with stellar-flow |
| **General** | Anything that doesn't fit the above |

### GitHub Issues

For confirmed bugs and feature requests only — not for support questions.

- [Report a bug](https://github.com/stellarbuild/stellar-flow/issues/new?template=bug_report.yml)
- [Request a feature](https://github.com/stellarbuild/stellar-flow/issues/new?template=feature_request.yml)
- [Report a documentation issue](https://github.com/stellarbuild/stellar-flow/issues/new?template=documentation_issue.yml)

For private matters or security concerns, see the
[Security Policy](SECURITY.md) — reports go through GitHub's private
vulnerability reporting tool, not email.

---

## Stellar Ecosystem Resources

If your question is about the Stellar network itself rather than this
library, the following resources may help:

| Resource | Link |
|---|---|
| Stellar Developers Portal | [developers.stellar.org](https://developers.stellar.org) |
| Stellar JavaScript SDK | [github.com/stellar/js-stellar-sdk](https://github.com/stellar/js-stellar-sdk) |
| Stellar Stack Exchange | [stellar.stackexchange.com](https://stellar.stackexchange.com) |
| Horizon API Reference | [developers.stellar.org/api](https://developers.stellar.org/api) |
| Stellar Laboratory | [lab.stellar.org](https://lab.stellar.org) |
| Soroban Documentation | [soroban.stellar.org](https://soroban.stellar.org) |

---

## Response Time Expectations

This is currently a single-maintainer project, so these are best-effort
targets rather than guaranteed response times:

| Channel | Expected Response |
|---|---|
| GitHub Discussions (Q&A) | Best effort, typically within a week |
| GitHub Issues | Best effort, typically within a week for initial triage |

Response times may be longer during busy periods. Community members are also
welcome to help answer questions in Discussions once the project has an
active contributor base.
